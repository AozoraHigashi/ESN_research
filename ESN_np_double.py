...
class ESN_mult():
    def __init__(self,N,sigma=0.1,p=1,pin=1,rho=0.9,rseed=0,uC=0,dim=2,shiftreg=False,idWin=False):
        ...
        if shiftreg:
            ...
            ただし、列も一つずつずれる
            """
            Win = np.zeros(shape=(N,dim), dtype=np.float128)
            pos = 0
            for i in range(dim):
                Win[pos][i]=1
                pos += lengths[i]
            W = np.zeros(shape=(N,N), dtype=np.float128)            
            d = np.ones(int(lengths[0]-1), dtype=np.float128)
    ...
    def run_washout(self,u,Two,actf=None, f=np.tanh,X0=[]): 
        ...
        T = len(u[0])
        X = np.zeros((T,self.N), dtype=np.float128)
        X[0] = X0
        for t in range(1,T):
            X[t] = f(self.W @ X[t-1] + self.Win@u[:,t-1])     #be careful with the expression
        
        Xwo = np.hstack([X[Two:],np.ones((T-Two,1), dtype=np.float128)])
        return Xwo
